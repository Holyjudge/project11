...
class Ui_OutputDialog(QDialog):
    ...
    def face_rec_(self, frame, encode_list_known, class_names):
        ...
        def mark_attendance(name):
            ...
            :return:
            """
            if self.PresentButton.isChecked():
                if self.PresentButton.isChecked():
                    self.PresentButton.setEnabled(False)
                    with open('Attendance.csv', 'a') as f:
                        if (name != 'unknown'):
                            buttonReply = QMessageBox.question(self, 'Welcome ' + name, 'Are you getting In?',
                                                               QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
                            if buttonReply == QMessageBox.Yes:

                                date_time_string = datetime.datetime.now().strftime("%y/%m/%d %H:%M:%S")
                                f.writelines(f'\n{name},{date_time_string}')
                                self.PresentButton.setChecked(False)
                                ...
                                # print(self.Time1)
                                self.PresentButton.setEnabled(True)
                            else:
                                print('Not clicked.')
                                self.PresentButton.setEnabled(True)
